...
$pengajuan_semua = query("SELECT * FROM pengajuan2 WHERE status ='Disetujui'");

// if (isset($_POST["cari"])) {
//   $pengajuan_semua = cari_menunggu_verifikasi($_POST["keyword"]);
          ...
          <div class="col-14">
            <div class="card">
              <div class="card-header mb-5">
                <h3 class="card-title" colspan="14">Tabel Peminjaman Ruangan <span class="text-warning font-weight-bold">(Menunggu Verifikasi)</span></h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                ...
                echo "
                    <script>
                        ...
                        document.location.href = 'menunggu-verifikasi.php';
                    </script>
                    ";
                <?php die; ?>
                <?php endif; ?>  
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <?php
                    ...
                    <tr>
                      ...
                      <td>
                        <a href="#" id="btn-verifikasi-<?= $row["kode_peminjaman"]; ?>"onclick="confirmVerif(<?= $row['id']; ?>,'<?= $row['kode_peminjaman'];?>');" class="btn btn-sm btn-success">
                              <i class="fas fa-check-circle"></i> Setuju
                          </a>
                          <a href="#" id="btn-tolak-verifikasi-<?= $row["kode_peminjaman"]; ?>" onclick="confirmTolakVerif(<?= $row['id']; ?>,'<?= $row['kode_peminjaman'];?>');" class="btn btn-sm btn-danger">
...
<script>
        function confirmVerif(id,kode_peminjaman) {
      const modal = `
        <div class="modal fade" id="modal-verif-${id}" tabindex="-1" aria-labelledby="modal-verif-label-${id}" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="modal-verif-label-${id}">Konfirmasi Persetujuan</h5>
                <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                
              </div>
              <div class="modal-body">
                Apakah anda ingin memverifikasi semua pengajuan dengan kode peminjaman ${kode_peminjaman}?
              </div>
              <div class="modal-footer">
                <a href="verifikasi-pengajuan.php?kode_peminjaman=${kode_peminjaman}" class="btn btn-primary">Ya, semua</a>
                <a href="verifikasi-pengajuan.php?id=${id}" class="btn btn-secondary">Tidak, hanya yang dipilih</a>
              </div>
            </div>
          </div>
        </div>
      ...
      // Tampilkan modal
      $("body").append(modal);
      $("#modal-verif-" + id).modal("show");

      // Hapus modal setelah ditutup
      $("#modal-verif-" + id).on("hidden.bs.modal", function () {
        $(this).remove();
      });
    }

    function confirmTolakVerif(id,kode_peminjaman) {
      const modal = `
        <div class="modal fade" id="modal-tolak-verif-${id}" tabindex="-1" aria-labelledby="modal-tolak-verif-label-${id}" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="modal-tolak-verif-label-${id}">Konfirmasi Persetujuan</h5>
                <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                
              </div>
              <div class="modal-body">
                Apakah anda yakin ingin menolak verifikasi semua pengajuan dengan kode peminjaman ${kode_peminjaman}?
              </div>
              <div class="modal-footer">
                <a href="tolak-verifikasi-pengajuan.php?kode_peminjaman=${kode_peminjaman}" class="btn btn-primary">Ya, semua</a>
                <a href="tolak-verifikasi-pengajuan.php?id=${id}" class="btn btn-secondary">Tidak, hanya yang dipilih</a>
              </div>
            </div>
          </div>
        </div>
      ...
      // Tampilkan modal
      $("body").append(modal);
      $("#modal-tolak-verif-" + id).modal("show");

      // Hapus modal setelah ditutup
      $("#modal-tolak-verif-" + id).on("hidden.bs.modal", function () {
        $(this).remove();
      });
    }
